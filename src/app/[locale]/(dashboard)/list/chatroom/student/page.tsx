...
export default function StudentChatroomPage() {
  ...
  const pendingInvitations: (Notification & { actionUrl: string })[] = notifications.filter(
    (n: Notification): n is Notification & { actionUrl: string } => n.type === 'session_invite' && !!n.actionUrl && !n.read
  );
  ...
  if (!user) {
      return (
        <div className="min-h-screen flex items-center justify-center bg-background">
            <p className="text-muted-foreground">Utilisateur non trouvé. Redirection...</p>
        </div>
      );
  }
            ...
            <Card className="border-l-4 border-l-green-500 bg-gradient-to-r from-white to-green-50/30">
              ...
              <CardContent>
                <div className="space-y-4">
                  {pendingInvitations.map((invitation: Notification & { actionUrl: string }) => (
                    <div
                      key={invitation.id}
