// src/app/[locale]/(dashboard)/list/classes/page.tsx
...
import { ArrowLeft, PlusCircle, Trash2 } from "lucide-react"; // Ajout de Trash2
        ...
        {classesInGrade.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {classesInGrade.map((classItem) => (
              <Link key={classItem.id} href={`/${locale}/list/classes/${classItem.id}`} passHref>
      ...
      {grades.length === 0 ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {grades.slice(0, 4).map((grade) => ( 
            <Card key={grade.id} className="hover:shadow-xl transition-shadow duration-300 bg-card hover:bg-muted/30 h-full flex flex-col relative group">
