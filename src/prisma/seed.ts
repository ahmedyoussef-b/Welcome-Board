...
async function main() {
  ...
  for (const subject of coreSubjects) {
    for (let i = 1; i <= 3; i++) {
      const sanitizedSubjectName = subject.name.toLowerCase().replace(/\s+/g, '');
      const user = await prisma.user.create({ data: { email: `prof.${sanitizedSubjectName}${i}@example.com`, username: `prof.${sanitizedSubjectName}${i}`, password: hashedPassword, role: Role.TEACHER, name: `Prof ${subject.name} ${i}`, active: true } });
