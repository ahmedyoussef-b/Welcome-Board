  ...
  return (
    <div className={cn("group h-80 w-64 [perspective:1000px]", className)}>
        <div className="relative h-full w-full rounded-xl shadow-xl transition-all duration-500 [transform-style:preserve-3d] group-hover:[transform:rotateY(180deg)]">
            {/* Front */}
            <div className="absolute inset-0 flex flex-col items-center justify-center rounded-xl bg-card border text-card-foreground p-6 text-center [backface-visibility:hidden]">
                <Icon className="w-16 h-16 text-primary mb-4" />
                <h3 className="text-2xl font-bold">{title}</h3>
                <p className="text-sm text-muted-foreground mt-2">Passez la souris pour en savoir plus</p>
            </div>
            {/* Back */}
